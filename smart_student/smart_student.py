...
class AppState(rx.State):
    ...
    async def download_summary_pdf(self):
        try:
            pdf_bytes = resumen_logic.generar_resumen_pdf_bytes(
                self.resumen_content, self.puntos_content, self.selected_curso, self.selected_libro, self.selected_tema
            )
            if pdf_bytes:
                rx.download(pdf_bytes, filename=f"Resumen_{self.selected_tema}.pdf")
            else:
                self.error_message_ui = "Error al generar PDF."
        except Exception as e:
            self.error_message_ui = f"Error al descargar PDF: {e}"
    ...
    async def submit_evaluation(self):
        try:
            result = eval_logic.calcular_resultado_logica(self.eval_preguntas, self.eval_user_answers)
            if result:
                ...
                self.error_message_ui = "Error al calcular resultado de la evaluación."
        except Exception as e:
    ...
    async def load_stats(self):
        ...
        try:
            ...
            self.error_message_ui = f"Error al cargar historial: {e}"
        finally:
