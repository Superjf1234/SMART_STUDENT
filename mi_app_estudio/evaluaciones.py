...
from mi_app_estudio.mi_app_estudio import AppState, BACKEND_AVAILABLE, config_logic, eval_logic, db_logic
...
class EvaluationState(AppState):
    ...
    def eval_progress(self) -> float:
        if self.eval_total_q == 0:
            return 0.0
        return ((self.eval_current_idx + 1) / self.eval_total_q) * 100.0
    ...
    async def generate_evaluation(self):
        if not self.resumen_content or len(self.resumen_content.strip()) < MIN_RESUMEN_LENGTH:
            self.error_message_ui = f"Debes generar un resumen con al menos {MIN_RESUMEN_LENGTH} caracteres."
            yield
            return
        if not BACKEND_AVAILABLE:
            self.error_message_ui = "El servicio de evaluación no está disponible."
            yield
            return
        ...
        try:
            ...
            if isinstance(result, dict) and result.get("status") == "EXITO":
                ...
                if isinstance(preguntas, list) and preguntas:
                    ...
                    self.error_message_ui = "No se pudieron generar preguntas."
                    self.is_eval_active = False
            else:
                self.error_message_ui = (
            ...
            self.error_message_ui = "El sistema de evaluación no está disponible."
            print(f"ERROR: {ae}")
        except Exception as e:
            import traceback
            self.error_message_ui = "Error inesperado al generar la evaluación."
