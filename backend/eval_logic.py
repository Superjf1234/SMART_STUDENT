...
def generar_preguntas(libro, tema, texto_pdf):
    ...
    prompt_alt = (
        ...
        f"Incluye una 'Explicación:' breve para cada una. IMPORTANTE: Las explicaciones deben presentar los hechos de forma directa sin usar frases como 'El texto afirma que' o referencias al texto. Formato ESTRICTO requerido:\n"
        f"Pregunta X: [Texto pregunta]\na) [Opción a]\nb) [Opción b] (correcta)\nc) [Opción c]\nd) [Opción d]\nExplicación: [Texto explicación directa sin referencias al texto]\n\n"
    ...
    prompt_vf = (
        ...
        f"Marca UNA opción (Verdadero o Falso) con '(correcta)'. Incluye 'Explicación:' sin usar frases como 'El texto afirma que' o referencias al texto.\nFORMATO ESTRICTO:\n"
    ...
    prompt_sm = (
        ...
        f"Formato: 4 alternativas (a, b, c, d), marca CADA correcta con '(correcta)'. Incluye 'Explicación:' sin usar frases como 'El texto afirma que' o referencias al texto.\nFORMATO ESTRICTO:\n"
        f"Pregunta X: [Texto pregunta]\na) [Opción a] (correcta)\nb) [Opción b]\nc) [Opción c] (correcta)\nd) [Opción d]\nExplicación: [Texto explicación directa sin referencias al texto]\n\n"
        f"(Repetir para 5 preguntas)\nTexto base:\n{texto_pdf}"
    )
    ...
    try:
        ...
        for tipo, texto in preguntas_texto.items():
            if isinstance(texto, str) and texto.strip() and "Error" not in texto:
                ...
                print(f"DEBUG: Preguntas parseadas para tipo {tipo}: {len(parseadas)}")
                preguntas_parseadas.extend(parseadas)
        if len(preguntas_parseadas) < 15:
            print(f"WARN: Solo se generaron {len(preguntas_parseadas)} preguntas válidas. Rellenando con preguntas adicionales.")
            while len(preguntas_parseadas) < 15:
                preguntas_parseadas.append({
                    "tipo": "relleno",
                    "pregunta": "Pregunta de relleno. Por favor, ignora esta pregunta.",
                    "alternativas": [
                        {"letra": "a", "texto": "Alternativa 1"},
                        {"letra": "b", "texto": "Alternativa 2"},
                        {"letra": "c", "texto": "Alternativa 3"},
                        {"letra": "d", "texto": "Alternativa 4"}
                    ],
                    "correcta": "a",
                    "explicacion": "Esta es una pregunta de relleno generada automáticamente."
                })
