...
def generar_mapa_pdf_bytes(mermaid_code=None, tema="", curso="", libro="", html_url=None, username=None):
    """
    ...
    Args:
        ...
        username (str): Nombre de usuario para registrar estadísticas (opcional)
    ...
    # Registrar la creación del mapa conceptual para estadísticas si se proporciona un usuario
    if username:
        try:
            from .user_stats import increment_map_count
            increment_map_count(username, tema)
            print(f"INFO (map_logic): Registrado mapa conceptual para usuario '{username}' sobre '{tema}'")
        except ImportError:
            print("WARN (map_logic): No se pudo importar user_stats para registrar estadísticas")
        except Exception as stats_error:
            print(f"WARN (map_logic): Error al registrar estadísticas de mapa: {stats_error}")
